...
{
    public partial class TeacherInfoView : UserControl, INotifyPropertyChanged
    {
        ...
        #region 基本信息加载
        private void LoadTeacherBasicInfo()
        {
            try
            {
                string teacherID = GlobalUserState.LinkedID;
                if (string.IsNullOrEmpty(teacherID))
                {
                    MessageBox.Show("未获取到教师信息！", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
                ...
                var dt = ExecuteQuery(query, new SqlParameter("@teacherID", teacherID));
                if (dt.Rows.Count > 0)
                {
            ...
            {
                MessageBox.Show($"加载教师信息失败: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void QueryPersonalSchedule(object sender, RoutedEventArgs e)
        {
            ...
            SELECT 
                ...
                ps.Classroom,
                CONCAT(ps.EnrolledCount, '/', ps.Capacity) as StudentCount
